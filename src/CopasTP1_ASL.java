    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < Q; i++) {
            ...
            if (query.equals("A")) {
                ...
                int waktuSekarang = querySekarang;
                int sisaKesabaran = pelanggan.KesabaranPelanggan - (waktuSekarang - pelanggan.queryMasuk);
                ...
                // Mencari ikan paling mahal yang dapat dibeli
                int ikanDibeli = -1;
                for (int k = HargaIkan.size() - 1; i >= 0; k--) {
                    if (pelanggan.UangDimiliki >= HargaIkan.get(k)) {
                        ikanDibeli = HargaIkan.get(i);
                        break;
                    }
                }
                ...
                // Diskon jika uang pas
                if (pelanggan.UangDimiliki == ikanDibeli && !kupon.isEmpty()) {
                    int diskon = kupon.pop();
                    int hargaSetelahDiskon = ikanDibeli - diskon;
                    if (hargaSetelahDiskon <= 0) {
                        pelanggan.UangDimiliki = 1;
                    } else {
                        pelanggan.UangDimiliki -= hargaSetelahDiskon;
                    }
                } else {
                    pelanggan.UangDimiliki -= ikanDibeli;
                }
            
                // Sisa uang masuk ke stack
                if (pelanggan.UangDimiliki > 0) {
                    kupon.push(pelanggan.UangDimiliki);
                }
            
                // Reset kesabaran pelanggan dan tambahkan kembali ke antrian
                pelanggan.KesabaranPelanggan = sisaKesabaran;
                pelanggan.queryMasuk = waktuSekarang;
                antrian.add(pelanggan);
            
                // Keluarkan uang yang digunakan untuk membeli ikan
                out.println(pelanggan.UangDimiliki);
            }
