...
public class TP1_5ygsalahB {
        ...
        for (int i = 0; i < Q; i++) {
            ...
            if (query.equals("A")) {
                ...
                if (antrian.isEmpty()) {
                    ...
                    Pelanggan pelangganTerdepan = antrian.poll(); // Ambil pelanggan terdepan dari antrian
                    int indexIkan = findMostExpensiveFishAffordable(pelangganTerdepan.UangDimiliki);
            
                    if (indexIkan == -1) {
                        // Tidak ada ikan yang dapat dibeli
                        out.println(pelangganTerdepan.id);
                        pelangganToko.remove(pelangganTerdepan.id); // Hapus pelanggan dari daftar pelanggan toko
                    } else {
                        int hargaIkan = HargaIkan.get(indexIkan);
                        int kembalian = pelangganTerdepan.UangDimiliki - hargaIkan;
            
                        // Jika uang pas dan ada kupon, gunakan kupon teratas
                        if (kembalian == 0 && !kupon.isEmpty()) {
                            kembalian -= kupon.pop(); // Gunakan kupon
                        }
            
                        if (kembalian < 0) {
                            kembalian = 1; // Jika setelah potongan kupon uangnya menjadi kurang dari 1, bayar 1
                        }
            
                        // Cetak uang kembalian
                        out.println(kembalian);
            
                        // Update pelanggan dengan sisa uang dan masukkan ke antrian belakang
                        pelangganTerdepan.UangDimiliki = kembalian;
                        pelangganTerdepan.queryMasuk = querySekarang;
            
                        // Jika pelanggan masih punya uang setelah pembelian, masukkan ke antrian belakang
                        if (pelangganTerdepan.UangDimiliki > 0) {
                            antrian.add(pelangganTerdepan);
                        }
                    }
                }
    ...
    public static int findMostExpensiveFishAffordable(int budget) {
    int left = 0, right = HargaIkan.size() - 1, result = -1;

    while (left <= right) {
        int mid = left + (right - left) / 2;

        if (HargaIkan.get(mid) <= budget) {
            result = mid; // Simpan index ikan yang bisa dibeli
            left = mid + 1; // Coba cari ikan yang lebih mahal yang masih bisa dibeli
        } else {
            right = mid - 1;
        }
    }
