...
public class TP1 {
            ...
            if (query.equals("A")) {
                ...
                if (antrian.isEmpty()) {
                    ...
                    if (indexIkan == -1) {
                        ...
                        // Cuman pelanggan yang bisa beli ikan yg bisa dapet diskon
                        if (!kupon.isEmpty()) {
                            int diskon = kupon.pop();
                            hargaIkanDiskon = hargaIkan - diskon;
            
                            // Kalau harga ikan <=0 set jadi 1
                            if (hargaIkanDiskon <= 0) {
                                hargaIkanDiskon = 1;
                            }
                        }
            
                        if (pelangganPalingDepan.UangDimiliki >= hargaIkanDiskon) {
                            int sisaUang = pelangganPalingDepan.UangDimiliki - hargaIkanDiskon; // beli ikan (kurangin uang dengan ikan yg udah didiskon)
    ...
    public static int cariIkanTermahal(int uangSekarang) {
        int kiri = 0, kanan = HargaIkan.size() - 1, hasil = - 1;
